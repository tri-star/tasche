...
**orvalのpatchバージョン更新(例: 8.19.0→8.20.0)の場合**、CIの`frontend-openapi`ジョブ(`git status --porcelain -- packages/frontend/src/api/generated` で差分を検知して失敗させる仕組み)は、生成ファイル先頭の `Generated by orval vX.Y.Z` というバージョンコメントが変わるだけでほぼ確実に失敗する(PR #79で確認、8.19.0→8.20.0、61ファイル全てで1行のみの差分)。これはorval自体やAPI定義の破壊的変更ではなく、単に「コミット済みの生成物のバージョン表記が古い」というだけなので、`pnpm --filter @tasche/frontend openapi:update` を実行して再コミットすれば解決する。orvalのマイナー/パッチ更新PRでこのジョブが落ちていたら、まずこのケースを疑って良い。

...
## 例外階層の既知ギャップ（TCH-62 で確認）

- `main.py` には `RequestValidationError`（Pydantic の `Field(ge=/le=/max_length=` 等）専用の `@app.exception_handler` が存在しない。そのため Pydantic 起因のスキーマ制約違反は FastAPI デフォルトの `{"detail": [...]}` 形式・422 で返り、`docs/api-design.md` が定義する共通エラー契約 `{"error": {"code", "message"}}` の対象外になっている（ドキュメントと実装の不一致）。
- この非対称性は `week_start_hour`(schemas/week.py)、`timezone`(schemas/setting.py)、`code`/`code_verifier`(schemas/auth.py)、`actual_units`(schemas/record.py) 等、複数のスキーマですでに存在する既存パターン。TCH-62（goal.py に `le=999.9`/`max_length` 追加）はこの既存パターンに追従しただけで、新規に持ち込んだ問題ではない。
- フロントエンドは現状 `error.code` を分岐に使っておらず（`TasksPage.tsx` 等は HTTP ステータスのみで一律メッセージ表示）実害はないが、契約としては壊れている。是正するなら `main.py` に `RequestValidationError` ハンドラを追加して統一フォーマットに変換するのが低コスト。
- 🟡 未修正の指摘事項（複数レビューで継続）。

## リクエスト/レスポンス共用スキーマの結合リスク（TCH-62 で確認）

- `schemas/goal.py` の `DailyTargets`/`DailyAvailableUnits` はリクエスト（`GoalsUpdate`）とレスポンス（`GoalResponse`/`GoalsResponse`/`PreviousGoalsResponse`）の両方で共用されている。
- `services/goal.py` の `_build_goal_responses()`/`_build_daily_available_units()` は DB の既存値からこれらのモデルを直接構築するため、書き込み側のバリデーション強化（例: `le=999.9` 追加）が読み取り専用の GET パスにも波及し、既存データが新しい上限を超えていた場合 GET が pydantic ValidationError で素の 500 になり得る。
- 999.9 という上限は現実的な業務データ量から乖離しており実害の可能性は低いが、一般原則として Request 用 / Response 用モデルの分離が望ましい（`docs/folder-structure.md` の schemas/ 方針とも整合）。
- 🟡 未修正の指摘事項（コード品質レビューで先行指摘、アーキテクチャレビューでも同意）。

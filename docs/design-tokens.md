...
完了率ヒートマップのように「段階を色で識別する」UIは、セマンティックトークン化が難しいため、
`dark:` バリアントを併記して対応します:

```tsx
// OK: ヒートマップの段階色（dark:バリアント併記）
if (rate === 0) return "bg-red-200 dark:bg-red-900/50"
if (rate < 50) return "bg-red-100 dark:bg-red-900/30"
if (rate < 80) return "bg-yellow-100 dark:bg-yellow-900/30"
if (rate < 100) return "bg-yellow-50 dark:bg-yellow-900/20"
...
| `tasche-text` | → `text-foreground` に置換済み | アプリ本体では使わない |
| `tasche-textSub` | → `text-muted-foreground` に置換済み | アプリ本体では使わない |
| `tasche-textMuted` | → `text-muted-foreground` に置換済み | アプリ本体では使わない |

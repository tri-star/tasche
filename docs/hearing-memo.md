...
> **[2026-04-23 補足] 認証方式の変更**
>
> 本ヒアリングメモ（初回作成時点）では認証基盤として Auth0 を利用する想定でしたが、
> TCH-26 にて UI を独自にカスタマイズしたい等の理由から、以下に方針変更しました。
>
> - 認証方式: **Google OAuth 2.0（BFF 型構成 + PKCE）の直接連携**（Auth0 は利用しない）
> - 対応プロバイダ: MVP では Google のみ（GitHub は MVP 対象外）
> - 詳細は `docs/mvp.md`、`docs/api-design.md`、`docs/screens.md` を参照
>
> 以下の記述（ヒアリング時の回答内容）は当時の記録として残してあります。最新の設計意図とは
> 差異があるため注意してください。
...
| タイムゾーン         | 自動検出                                                                |
| 目標未設定時の導線   | 当週未設定時はダッシュボードに空状態を表示、目標設定画面では直近の過去週の設定をデフォルト値として表示 |
| 過去記録の修正       | 今週中のみ可能                                                          |

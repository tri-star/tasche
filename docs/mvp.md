...
| 機能ID | 機能名           | 説明                                      | 優先度 |
| ------ | ---------------- | ----------------------------------------- | ------ |
| AUTH-1 | ソーシャルログイン | Google OAuth 2.0 を利用したログイン（MVPではGoogleのみサポート） | 必須   |
...
| GOAL-5 | 目標未設定時の導線     | 当週の目標未設定時はダッシュボードに空状態を表示、目標設定画面では直近の過去週の設定をデフォルト値として表示 | 必須   |
...
| RECORD-2 | 0.1単位入力      | 0.1ユニット刻みで実績を入力可能（スピン操作は0.5刻み、手入力では任意の小数も可能） | 必須   |
...
| カテゴリ     | 機能                                           |
| ------------ | ---------------------------------------------- |
| 目標設定     | 目標変更（週途中）：週の途中でも目標ユニット数・ユニット時間を変更可能 |
| 統計・分析   | 先週の達成状況表示                             |
| 統計・分析   | 1か月の達成状況表示                            |
...
| 項目           | 技術/ツール                                      |
| -------------- | ------------------------------------------------ |
| 認証方式       | Google OAuth 2.0 (OIDC) + PKCE（BFF型構成）      |
| 対応プロバイダ | Google                                           |

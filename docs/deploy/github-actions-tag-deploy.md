...
3. **frontend prod を先にデプロイする** (`release/frontend-prod/YYYY-MM-DD-1`)
4. デプロイ完了後に Distribution ID を取得する:
   ```bash
   aws cloudformation describe-stacks \
     ...
     --output text
   ```
5. `packages/backend/infra/samconfig.toml` の prod セクションの `CloudFrontDistributionId=REPLACE_WITH_PROD_CLOUDFRONT_DISTRIBUTION_ID` を取得した ID に差し替えてコミット・push する
6. **backend prod をデプロイする** (`release/backend-prod/YYYY-MM-DD-1`)

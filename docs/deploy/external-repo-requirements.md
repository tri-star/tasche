...
そのため、**backend を先にデプロイ**する必要がある。初回は以下の順序でデプロイする:

1. 外部リポで上記リソースをすべて整備
2. GitHub で `prod` environment の設定を完了
3. **backend prod を先にデプロイ** (`release/backend-prod/YYYY-MM-DD-1` タグを push)
   - この時点では `CloudFrontDistributionId` はプレースホルダーのままで問題ない
   - デプロイ完了後、SSM `/tasche/prod/sam/backend-function-url` が自動作成される
4. **frontend prod をデプロイ** (`release/frontend-prod/YYYY-MM-DD-1` タグを push)
5. デプロイ完了後、CloudFormation Outputs から `DistributionId` を取得:
   ```bash
   aws cloudformation describe-stacks \
     ...
     --output text
   ```
6. 取得した ID を `packages/backend/infra/samconfig.toml` の prod セクション
   `CloudFrontDistributionId` に設定してコミット
7. **backend prod を再デプロイ** (`release/backend-prod/YYYY-MM-DD-2` タグを push)

...
**用途**: 実績ユニット数の入力（0.5刻みスピン操作または手入力）

**要件**:
- 0.5単位でスピン増減（上下ボタン・上下キー）
- 手入力による任意の小数値入力も可能（0.5刻み以外の端数も許容）
- キーボード操作（上下キー・Home/End）
- 最小値: 0、最大値: 業務上限を max prop で指定
- 小数点第1位まで表示（step の小数桁数に依存）
...
**実装方針**: 独自実装（shadcn/ui の Button + text input を組み合わせ）。フォーカス中は入力中の draft 値を表示し、blur 時に確定・正規化する

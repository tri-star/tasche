...
- **コンテナイメージ Lambda には Lambda Layer をアタッチできない** という制約があり、当初想定していた "Parameters and Secrets Lambda Extension" は使えない。
  - 代わりに boto3 を直接呼び出し、`functools.lru_cache` でモジュールスコープのキャッシュを構成する方式を採用 (`packages/backend/src/tasche/core/config.py`)。
  - ウォーム呼び出し時に SecretsManager API を叩かないため、Extension とほぼ同等の体感性能。

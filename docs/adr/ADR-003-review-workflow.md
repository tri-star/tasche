...
さらに、`pull_request` 上で PR branch の workflow / prompt / agent / skill をそのまま実行すると、
review workflow 自体を PR で改ざんできてしまう。
trusted な review 基盤と untrusted な review 対象コードを分離する必要がある。
...
### 3.1 trusted workflow と review 対象コードを分離する

review workflow は `pull_request_target` で起動し、workflow 定義、agent、skill、prompt builder、review docs は default branch の trusted code を使う。
PR head は privileged job で checkout しない。

changed file の確認は次のように分離する。

- changed file の PR 版: `git show <head_sha>:<path>`
- changed file の base 版: `git show <base_sha>:<path>`
- unchanged な周辺コードや docs: trusted workspace のファイルを読む

これにより、review ルールや prompt 自体は PR から改ざんされず、review 対象コードだけを PR head から参照できる。

### 3.2 fork PR は AI review の対象外とする

`pull_request_target` は privileged trigger であり、GitHub 公式の secure use reference でも untrusted checkout を避けるよう求められている。
そのため、初期版では fork PR に対して AI review は実行しない。

fork PR は次の扱いとする。

- summary comment は更新する
- verdict は `human-review` とする
- line comment は作らない
- S3 への review result upload は行わない

### 3.3 レビュー専用の instructions は `.github/agents/pr-review.agent.md` に集約する
...
### 3.4 関連 docs の探索は skill と script を優先して解決する
...
- workflow は trusted step でこの script を先に実行し、review agent は生成済み metadata / diff / candidate files を読む
- 手動実行時だけ review agent からこの skill を呼び出してもよい
...
そのため、初期版では review agent の front matter に skill 専用設定は追加しない。
また、CI の privileged 実行では Python 実行権限を agent に渡さず、`git diff` と `git show` を中心に読ませる。
...
- workflow / prompt / agent / skill の改ざんを PR 側から行いにくくなる
...
- fork PR は AI review の自動判定対象外になる
...
review-evaluator (Lambda) が S3 に蓄積する「レビュールール」を、tasche 側の AI Review ワークフローで消費する。

#### 責務の分担

| 責務 | 担当 |
|------|------|
| ルールの生成・重複判定・削除 | Lambda (review-evaluator) |
| ルールの取得・プロンプト注入・使用マーカー付与 | tasche (pr-ai-review.yml) |

#### S3 契約

- バケット: `AI_REVIEW_S3_BUCKET`（既存 secret を流用）
- プレフィックス: `rules/package=<frontend\|backend>/`
- ルール JSON スキーマ: `rule_id`(uuid) / `name` / `package` / `category` / `body` / `recent_usage`
- 使用マーカー: `<!-- RuleId: <uuid> -->`（`rule_id` が安定キー。Lambda が LRU 判定に使用）

#### ワークフロー組み込みの設計判断

1. **AWS 認証の前倒し**: S3 アップロード用に末尾にあった OIDC 認証を、ルール取得のためレビュー実行前に移動。有効期限内で末尾のアップロードも同じ認証を流用。

2. **対象パッケージの判定**: `changed_files` から判定。`packages/frontend/` → frontend、`packages/backend/` → backend。どちらも該当しない場合は両方取得（汎用ルールのフォールバック）。

3. **耐障害性**: ルール取得ステップは `continue-on-error: true`。S3 未設定・ルールゼロ・取得失敗のいずれの場合もレビュー本体は続行し、既存挙動を壊さない。

4. **捏造防止**: `validate_pr_review_result.py` で `applied_rule_ids` を既知 uuid のみにフィルタリング。プロンプトで渡していないルール ID は結果 JSON に混入しない。

5. **マーカー付与箇所**: line comment と summary comment の両方に `<!-- RuleId: <uuid> -->` を埋め込む。

#### 前提（リポジトリ外の依存）

OIDC で assume する IAM ロールに、対象バケットの `s3:ListBucket`（prefix `rules/*`）と `s3:GetObject`（`rules/*`）の read 権限が必要。これは review-evaluator 側タスクと整合させること。

## Related Notes

- GitHub Actions は `pull_request_target.opened` と `pull_request_target.synchronize` で実行し、draft PR は対象外とする

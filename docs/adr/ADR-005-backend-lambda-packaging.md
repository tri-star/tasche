...
- パッケージは `packages/backend/Dockerfile` (本番用) を SAM CLI が build / push し、必要な ECR リポジトリも `sam deploy --resolve-image-repos` で自動作成する。
...
- **コンテナイメージ Lambda には Lambda 設定経由で Layer をアタッチできない**。
  ...
  - 本プロジェクトでは `packages/backend/scripts/fetch-lambda-extensions.sh` で Layer zip の署名 URL だけを解決し、`Dockerfile` の中間ステージで zip を download / unzip して `/opt` に焼き込んでいる。
...
- 2026-05-19 時点で ADOT / OpenTelemetry は `packages/backend/scripts/start-backend.sh` と `AWS_LAMBDA_EXEC_WRAPPER=/opt/otel-instrument` により実装済み。今後 exporter やサンプリング方針を変える場合も、起動ラッパーと環境変数の差し替えで追従できる。

...
**dev / prod 別に各 3 本**を用意。

| Secret 名 (例) | 値の形式 | 内容 |
|---|---|---|
| `tasche/dev/db-url` | plain string | Neon PostgreSQL の接続文字列 (`postgresql+asyncpg://user:pass@host/db?sslmode=require` 形式) |
| `tasche/dev/jwt-secret` | plain string | 自前発行 JWT の HS256 署名鍵 (32 文字以上のランダム値) |
| `tasche/dev/google-oauth` | JSON | `{"client_id": "...", "client_secret": "..."}` |
...
| `/tasche/dev/secrets/db-url-arn` | 2 の `tasche/dev/db-url` Secret の ARN |
| `/tasche/dev/secrets/jwt-secret-arn` | 2 の `tasche/dev/jwt-secret` Secret の ARN |
| `/tasche/dev/secrets/google-oauth-arn` | 2 の `tasche/dev/google-oauth` Secret の ARN |
...
3. SecretsManager Secret 3 本 (dev) を作成し、適切な値を設定

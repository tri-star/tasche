...
2. trusted workspace から `scripts/build_review_context.py` を実行して `tmp/review/` 配下に review context を生成する
3. changed file の PR 版は local workspace ではなく `git show <head_sha>:<path>` で参照する前提を維持する
4. 生成された `tmp/review/context.json` を確認する
5. 必須 review docs と候補ファイル一覧を review agent に渡す
...
- trusted workspace は判定ルールや unchanged context 用であり、changed file の PR 版そのものではない
